...
contour(
    range(0, 4; length=64), range(-3, 25; length=64), 
...
model_ad = LogDensityProblemsAD.ADgradient(ADTypes.AutoReverseDiff(; compile=true), model, x=[1., 1.])
nothing
```
...
binv = Bijectors.bijector(model) |> Bijectors.inverse
...
q_out, info, _ = AdvancedVI.optimize(
    alg, max_iter, model_ad, q_trans; show_progress=false
)
nothing
```
...
histogram2d(samples[1,:], samples[2,:], normalize=:pdf, nbins=32, xlabel="α", ylabel="β", xlims=(0,4), ylims=(-3,25))
...
```@example flow
plot([i.elbo for i in info_flow], xlabel="Iteration", ylabel="ELBO", ylims=(-30,Inf))
...
histogram2d(samples[1,:], samples[2,:], normalize=:pdf, nbins=64, xlabel="α", ylabel="β", xlims=(0,4), ylims=(-3,25))
