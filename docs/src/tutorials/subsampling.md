...
For the dataset, we will use a larger synthetic binary classification problem with 4096 data points and 48 features.
This is enough to make the effect of subsampling visible without relying on a network download.
...
We will use a batch size of 64, which results in `length(subsampling)` steps per epoch.
...
Recall that each epoch is `length(subsampling)` steps.
