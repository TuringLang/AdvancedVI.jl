...
function init(
    ...
    ad_prob = if capability < LogDensityProblems.LogDensityOrder{1}()
        ...
        if !(
            ...
            @info "The supplied target `LogDensityProblem` has a differentiation capability of $(capability) >= `LogDensityProblems.LogDensityOrder{1}()`. To make use of this, the `adtype` argument for AdvancedVI must be one of `AutoReverseDiff`, `AutoZygote`, `AutoMooncake`, or `AutoEnzyme` in reverse mode."
        end
