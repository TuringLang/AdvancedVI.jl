...
@testset "KLMinNaturalGradDescent" begin
    ...
    @testset "error low capability" begin
        ...
        alg = KLMinNaturalGradDescent(; n_samples=10, stepsize=1.0)
    ...
    @testset "subsampling" begin
        ...
        @testset "estimate_objective batchsize=$(batchsize)" for batchsize in [1, 3, 4]
            ...
            alg = KLMinNaturalGradDescent(; n_samples=10, stepsize=1e-3)
            alg_sub = KLMinNaturalGradDescent(; n_samples=10, stepsize=1e-3, subsampling)
        ...
        @testset "convergence capability=$(capability)" for capability in [1, 2]
            ...
            batchsize = 1
            subsampling = ReshufflingBatchSubsampling(1:n_data, batchsize)
            alg_sub = KLMinNaturalGradDescent(; n_samples=10, stepsize=1e-2, subsampling)
