...
@testset "interface optimize" begin
    ...
    @testset "default_rng" begin
        optimize(alg, T, model_ad, q0; show_progress=false)
    end
    ...
    @testset "callback" begin
        ...
        _, info, _ = optimize(alg, T, model_ad, q0; show_progress=false, callback)
        @test [i.test_value for i in info] == test_values
    end

    rng = StableRNG(seed)
    q_avg_ref, _, _ = optimize(rng, alg, T, model_ad, q0; show_progress=false)
    ...
    @testset "warm start" begin
        ...
        _, _, state = optimize(rng, alg, T_first, model_ad, q0; show_progress=false)
        q_avg, _, _ = optimize(rng, alg, T_last, model_ad, q0; show_progress=false, state)
